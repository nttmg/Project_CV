...
export default class Game extends Phaser.Scene {
  ...
  create() {
    ...
    this.listDuckLost = [];
    for(let i = 1; i < 4; i++) {
      this.listDuckLost.push(this.add.image(i * 45, 25, "icon").setDepth(10).setScale(0.8))
    }

    //create control scope
  ...
  createTarget() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      target.setDepth(4);
      target.setVelocity(Phaser.Math.Between(300, 400), 0, 10);
  ...
  handleShot(pointer) {
    ...
    this.targets.getChildren().forEach((target) => {
      if(Phaser.Geom.Rectangle.ContainsPoint(target.getBounds(), pointer.position)) {
        target.destroy();
